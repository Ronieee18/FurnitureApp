...
export default function NavbarWithMegaMenu() {
  ...
  return (
    <Navbar className="mx-auto w-full px-4 py-2 my-2">
      <div className="flex items-center justify-between text-blue-gray-900">
        <div className='cursor-pointer' onClick={()=>navigate('/')}>
          <img src="./logo.png" alt="" height={180} width={180} />
        </div>
       
        <div className="hidden lg:block">
          {/* <NavList /> */}
        </div>
        <div className="relative  w-full gap-2 md:w-max hidden md:flex max-md:hidden">
          <Input
            type="search"
            color="black"
        ...
        <IconButton variant="text" size="sm" color="blue-gray" className="ml-3" onClick={()=>navigate('/orders')}>
            <ShoppingCartIcon className="h-6 w-6 " />
          </IconButton>
          <div className="hidden gap-5 lg:flex">
          {!checkIsAuthenticated && (
            <Button variant="gradient" size="l"  onClick={() => navigate('/login')}>
              Log In / Sign Up
            </Button>
          )}
          {checkIsAuthenticated && (
            <>
              ...
              <img src={user?.avatar} alt="" height={40} width={40} className='rounded-full' />
